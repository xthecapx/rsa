...
export const api = {
  ...
  keyboard: (opts?: { modulus?: number; require_coprime?: boolean }) =>
    post<{ keys: KeyState[] }>("/api/keyboard", opts ?? {}),
  ...
  shor: {
    ...
    classical: (N: number, a: number, num_control?: number) =>
      post<Record<string, unknown>>("/api/shor/classical", {
        N,
        a,
        num_control,
      }),
  ...
  ibm: {
    batches: () =>
      get<{ batches: IbmBatchSummary[] }>("/api/ibm/batches"),
